...
class NotificationController {
  ...
  static Future<void> scheduleNewNotification(ScheduleModel data) async {
    bool isAllowed = await AwesomeNotifications().isNotificationAllowed();
    if (!isAllowed) isAllowed = await displayNotificationRationale();
    if (!isAllowed) return;
    ...
    await myNotifyScheduleInHours(
      ...
      nowDate: data.scheduleTime.toDate(),
      repeatNotif: data.repeat,
    );
  }

  static Future<void> cancelScheduleNotification(int notifId) async {
    bool isAllowed = await AwesomeNotifications().isNotificationAllowed();
    if (!isAllowed) isAllowed = await displayNotificationRationale();
    if (!isAllowed) return;
